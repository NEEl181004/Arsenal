        ...
        <div className="space-y-8 sm:space-y-12 animate-in fade-in duration-1000">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/5 pb-6 sm:pb-8 gap-6">
                ...
                <div className="md:hidden relative w-full group">
                    <div className="text-xs font-bold text-primary uppercase tracking-[0.4em] mb-3 ml-1">SELECT ENVIRONMENT</div>
                    <select 
                        value={activeTab}
                        onChange={(e) => setActiveTab(Number(e.target.value))}
                        className="w-full bg-white/[0.03] border border-white/10 p-4 pr-12 appearance-none text-xs font-bold text-white uppercase tracking-[0.2em] focus:outline-none focus:border-primary/50 transition-all cursor-pointer"
                    >
                        {data.map((tab, i) => (
                ...
                <div className="hidden md:flex flex-wrap gap-4">
                    {data.map((tab, i) => (
                        <button 
                            ...
                            className={`px-8 py-3 text-xs font-bold uppercase tracking-[0.3em] transition-all border ${
                                i === activeTab 
                ...
                <div className="flex flex-col items-start md:items-end gap-1">
                    <div className="text-xs font-bold text-white/10 uppercase tracking-[0.2em] sm:tracking-[0.4em] whitespace-nowrap">
                    ...
                    <div key={i} className="bg-white/[0.01] border border-white/5 hover:bg-white/[0.02] transition-all group overflow-hidden">
                        <div className="grid grid-cols-1 lg:grid-cols-2">
                            <div className="p-6 md:p-10 border-b lg:border-b-0 lg:border-r border-white/5 relative">
                                ...
                                <div className="text-sm font-bold text-white uppercase tracking-[0.2em] sm:tracking-[0.3em] border-l-2 border-primary pl-4 mb-6 break-words">
                                    STEP {step.id || `0${i+1}`}: {step.name.toUpperCase()}
                                </div>
                                <p className="text-sm text-white/50 font-light leading-relaxed pr-4 break-words">
                                    {step.desc}
                                </p>
                            </div>
                            <div className="flex-1 p-6 md:p-10 bg-black/40 flex items-center justify-between group/cmd relative">
                                <div className="flex items-start gap-4 sm:gap-6 font-mono text-xs sm:text-sm scroll-smooth">
